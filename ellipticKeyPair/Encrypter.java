...
import static EllipticKeyPair.Utilities.toBinary;
...
public class Encrypter {
    ...
    public Encrypter(HashMap<Integer, Point> charTable) {
        this.charTable = charTable;
    }

    public Matrix encrypt(String plainText) {
        Matrix mMatrix = createMatrix(plainText);
        int w = new BigInteger(Elliptic.PAD, Elliptic.getRandom()).intValue();
        int[] bits = Utilities.toBinary(Elliptic.getRandom().nextInt(1024), Elliptic.PAD * 2);
        int bit, i = 0;
        do {
            bit = bits[i];
            if (bit == 0) {
    ...
    private Matrix createMatrix(String plainText) {
        ...
        return Utilities.listToMatrix(bList);
    }
